  ...
  namespace bc127
  {
    ...
    void BC127Component::process_data(const String &data)
    {
      ...
      if (data.startsWith("OPEN_OK"))
      {
        ...
        // Encuentra los espacios en blanco
        int first_space = data.indexOf(' '); // El primer espacio después de "OPEN_OK "
        ...
        {
          // Extrae los valores en tres variables
      ...
      if (data.startsWith("CALL_END"))
      {
        ...
        ESP_LOGI(TAG, "%d, this->state");

        if ((this->state == BC127_CALL_IN_COURSE) || (this->state == BC127_CONNECTED))
        {
