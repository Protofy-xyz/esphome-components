...
void MKS42DComponent::set_speed(int speed, int acceleration, const std::string &direction) {
  ...
  uint16_t raw_speed = speed;  // 0–3000
  uint8_t byte2 = ((direction == "CCW") ? 0x80 : 0x00) | ((raw_speed >> 8) & 0x0F);
  uint8_t byte3 = raw_speed & 0xFF;
  uint8_t byte4 = acceleration & 0xFF;

  std::vector<uint8_t> data = {0xF6, byte2, byte3, byte4};
  uint8_t crc = this->can_id_;
  for (auto b : data) crc += b;
  data.push_back(crc & 0xFF);
