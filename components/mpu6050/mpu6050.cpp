...
void MPU6050Component::update() {
  ...
  // // ✅ Leer INT_STATUS para limpiar la interrupción
  // uint8_t status;
  // this->read_byte(0x3A, &status);

  this->status_clear_warning();
}
float MPU6050Component::get_setup_priority() const { return setup_priority::DATA; }

void MPU6050Component::configure_interrupt_() {
  ESP_LOGCONFIG(TAG, "Configuring motion interrupt...");

  // Enable motion interrupt
  this->write_byte(0x1A, 0x00);  // Disable DLPF
  this->write_byte(0x38, 0x40);  // Enable Motion interrupt + Data Ready
  this->write_byte(0x1F, this->motion_threshold_);  // MOT_THR
  this->write_byte(0x20, this->motion_duration_);   // MOT_DUR
  this->write_byte(0x6B, 0x00);  // Wake up device
  this->write_byte(0x6A, 0x00);  // Disable FIFO
  this->write_byte(0x69, 0x00);  // Disable FIFO/ I2C master
  this->write_byte(0x37, 0x20);  // INT pin active high, push-pull, latch until cleared
}
void MPU6050Component::clear_interrupt_latch() {
  uint8_t status;
  this->read_byte(0x3A, &status);
  ESP_LOGD(TAG, "INT_STATUS register read to clear latch: 0x%02X", status);
}
