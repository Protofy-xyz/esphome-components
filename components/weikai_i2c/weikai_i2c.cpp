...
/// @brief Computes the I2C bus's address used to access the component
...
void WeikaiComponentI2C::setup() {
  ...
  // Handle power pin: drive it before any I2C communication
  if (this->power_pin_ != nullptr) {
    this->power_pin_->setup();
    if (this->enable_on_boot_) {
      ESP_LOGCONFIG(TAG, "Enabling power pin for %s", this->get_name());
      this->power_pin_->digital_write(true);
      delay(100);  // wait for chip to power up and stabilize
    } else {
      ESP_LOGCONFIG(TAG, "Power pin configured but enable_on_boot is false for %s", this->get_name());
      return;  // skip setup if not enabling on boot
    }
  }
...
void WeikaiComponentI2C::dump_config() {
  ...
  if (this->power_pin_ != nullptr) {
    ESP_LOGCONFIG(TAG, "  Power pin: configured (enable_on_boot: %s)", this->enable_on_boot_ ? "true" : "false");
  }
  if (test_mode_) {
    ESP_LOGCONFIG(TAG,
                  "  Test mode: %d\n"
